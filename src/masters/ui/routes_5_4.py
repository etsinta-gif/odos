...
async def revenue_list(
    ...
    query = db.query(TRN_Revenue).filter(TRN_Revenue.is_active == True, TRN_Revenue.company_id == current_user.company_id)
    if case_id:
    ...
    revenues = query.all()
    for rev in revenues:
        case = db.query(TRN_Case).filter(TRN_Case.case_id == rev.case_id, TRN_Case.company_id == current_user.company_id).first()
        if case:
            rev.case_number = case.case_number
            lender = db.query(MST_Lender).filter(MST_Lender.lender_id == case.lender_id, MST_Lender.company_id == current_user.company_id).first()
...
async def revenue_add_form(
    ...
    cases_with_info = []
    for case in cases:
        customer = db.query(MST_Customer).filter(MST_Customer.customer_id == case.customer_id, MST_Customer.company_id == current_user.company_id).first()
...
async def revenue_detail(request: Request, revenue_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
    rev = db.query(TRN_Revenue).filter(TRN_Revenue.revenue_id == revenue_id).first()
    rev = _ensure_company_record(rev, current_user.company_id, "Revenue")
    case = db.query(TRN_Case).filter(TRN_Case.case_id == rev.case_id, TRN_Case.company_id == current_user.company_id).first()
    lender = db.query(MST_Lender).filter(MST_Lender.lender_id == case.lender_id, MST_Lender.company_id == current_user.company_id).first() if case else None
    return TEMPLATES.TemplateResponse(
...
async def revenue_edit_form(request: Request, revenue_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
    rev = db.query(TRN_Revenue).filter(TRN_Revenue.revenue_id == revenue_id).first()
    rev = _ensure_company_record(rev, current_user.company_id, "Revenue")
    cases = db.query(TRN_Case).filter(TRN_Case.is_active == True, TRN_Case.company_id == current_user.company_id).all()
    cases_with_info = []
...
async def revenue_update(
    ...
    existing = db.query(TRN_Revenue).filter(TRN_Revenue.revenue_id == revenue_id).first()
    _ensure_company_record(existing, current_user.company_id, "Revenue")
    scoped_case = db.query(TRN_Case).filter(TRN_Case.case_id == case_id, TRN_Case.company_id == current_user.company_id).first()
    if not scoped_case:
        raise HTTPException(status_code=404, detail="Case not found")
...
async def revenue_delete(revenue_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def revenue_calculate(case_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def commission_list(
    ...
    query = db.query(TRN_Commission).filter(TRN_Commission.is_active == True, TRN_Commission.company_id == current_user.company_id)
    if case_id:
        scoped_case = db.query(TRN_Case).filter(TRN_Case.case_id == case_id, TRN_Case.company_id == current_user.company_id).first()
        if not scoped_case:
            raise HTTPException(status_code=404, detail="Case not found")
    ...
    if connector_id:
        scoped_connector = db.query(MST_Connector).filter(MST_Connector.connector_id == connector_id, MST_Connector.company_id == current_user.company_id).first()
        if not scoped_connector:
    ...
    for comm in commissions:
        case = db.query(TRN_Case).filter(TRN_Case.case_id == comm.case_id, TRN_Case.company_id == current_user.company_id).first()
        if case:
            comm.case_number = case.case_number
        connector = db.query(MST_Connector).filter(MST_Connector.connector_id == comm.connector_id, MST_Connector.company_id == current_user.company_id).first()
        if connector:
            comm.connector_name = connector.full_name
    connectors = db.query(MST_Connector).filter(MST_Connector.is_active == True, MST_Connector.company_id == current_user.company_id).all()
    return TEMPLATES.TemplateResponse(
...
async def commission_detail(request: Request, commission_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
    comm = db.query(TRN_Commission).filter(TRN_Commission.commission_id == commission_id).first()
    comm = _ensure_company_record(comm, current_user.company_id, "Commission")
    case = db.query(TRN_Case).filter(TRN_Case.case_id == comm.case_id, TRN_Case.company_id == current_user.company_id).first()
    connector = db.query(MST_Connector).filter(MST_Connector.connector_id == comm.connector_id, MST_Connector.company_id == current_user.company_id).first()
    return TEMPLATES.TemplateResponse(
...
async def commission_edit_form(request: Request, commission_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
    comm = db.query(TRN_Commission).filter(TRN_Commission.commission_id == commission_id).first()
    comm = _ensure_company_record(comm, current_user.company_id, "Commission")
    cases = db.query(TRN_Case).filter(TRN_Case.is_active == True, TRN_Case.company_id == current_user.company_id).all()
    cases_with_info = []
...
async def commission_update(
    ...
    existing = db.query(TRN_Commission).filter(TRN_Commission.commission_id == commission_id).first()
    _ensure_company_record(existing, current_user.company_id, "Commission")
    scoped_case = db.query(TRN_Case).filter(TRN_Case.case_id == case_id, TRN_Case.company_id == current_user.company_id).first()
    if not scoped_case:
        raise HTTPException(status_code=404, detail="Case not found")
    scoped_connector = db.query(MST_Connector).filter(MST_Connector.connector_id == connector_id, MST_Connector.company_id == current_user.company_id).first()
...
async def commission_delete(commission_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def commission_calculate(case_id: int, current_user: SEC_User = Depends(get_current_user), db: Session = Depends(get_db)):
