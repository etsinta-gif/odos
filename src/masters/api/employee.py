...
router = APIRouter(prefix="/api/masters/employees", tags=["Masters"], dependencies=[Depends(get_current_user), Depends(require_company_id)])
...
def list_employees(
    ...
    if company_id is not None:
        query = query.filter(MST_Employee.company_id == company_id)
    if search:
        query = query.filter(
...
def get_employee(employee_id: int, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    employee = db.query(MST_Employee).filter(MST_Employee.employee_id == employee_id)
    if company_id is not None:
        employee = employee.filter(MST_Employee.company_id == company_id)
