...
router = APIRouter(prefix="/api/masters/expenses", tags=["Masters"], dependencies=[Depends(get_current_user), Depends(require_company_id)])
...
def list_expenses(
    ...
    if company_id is not None:
        query = query.filter(TRN_Expense.company_id == company_id)
    ...
    for exp in expenses:
        ...
        if exp.case_id:
            case = db.query(TRN_Case).filter(TRN_Case.case_id == exp.case_id)
            if company_id is not None:
...
def get_expense(expense_id: int, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    exp = db.query(TRN_Expense).filter(TRN_Expense.expense_id == expense_id)
    if company_id is not None:
        exp = exp.filter(TRN_Expense.company_id == company_id)
    exp = exp.first()
    if not exp:
...
def create_expense(expense_data: ExpenseCreate, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    category = db.query(MST_ExpenseCategory).filter(MST_ExpenseCategory.expense_category_id == expense_data.expense_category_id)
    if company_id is not None:
        category = category.filter(MST_ExpenseCategory.company_id == company_id)
...
def list_recurring_expenses(db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    recurring = db.query(TRN_RecurringExpense).filter(TRN_RecurringExpense.is_active == True)
    if company_id is not None:
        recurring = recurring.filter(TRN_RecurringExpense.company_id == company_id)
...
def update_recurring_expense(recurring_id: int, data: RecurringExpenseCreate, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    recurring = db.query(TRN_RecurringExpense).filter(TRN_RecurringExpense.recurring_expense_id == recurring_id)
    if company_id is not None:
        recurring = recurring.filter(TRN_RecurringExpense.company_id == company_id)
