...
router = APIRouter(prefix="/api/masters/cases", tags=["Masters"], dependencies=[Depends(get_current_user), Depends(require_company_id)])
...
def list_cases(
    ...
    if company_id is not None:
        query = query.filter(TRN_Case.company_id == company_id)
    if search:
        query = query.filter(
...
def create_case(case_data: CaseCreate, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    customer = db.query(MST_Customer).filter(MST_Customer.customer_id == case_data.customer_id)
    lender = db.query(MST_Lender).filter(MST_Lender.lender_id == case_data.lender_id)
    product = db.query(MST_Product).filter(MST_Product.product_id == case_data.product_id)
    if company_id is not None:
        customer = customer.filter(MST_Customer.company_id == company_id)
        lender = lender.filter(MST_Lender.company_id == company_id)
        product = product.filter(MST_Product.company_id == company_id)
