...
router = APIRouter(prefix="/api/masters/products", tags=["Masters"], dependencies=[Depends(get_current_user), Depends(require_company_id)])
...
def list_products(
    ...
    if company_id is not None:
        query = query.filter(MST_Product.company_id == company_id)
    if search:
        query = query.filter(
...
def get_product(product_id: int, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    product = db.query(MST_Product).filter(MST_Product.product_id == product_id)
    if company_id is not None:
