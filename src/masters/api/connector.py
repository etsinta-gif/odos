...
router = APIRouter(prefix="/api/masters/connectors", tags=["Masters"], dependencies=[Depends(get_current_user), Depends(require_company_id)])
...
def list_connectors(
    ...
    if company_id is not None:
        query = query.filter(MST_Connector.company_id == company_id)
    if search:
        query = query.filter(
...
def get_connector(connector_id: int, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    connector = db.query(MST_Connector).filter(MST_Connector.connector_id == connector_id)
    if company_id is not None:
