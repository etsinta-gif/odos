...
router = APIRouter(prefix="/api/masters/revenue", tags=["Masters"], dependencies=[Depends(get_current_user), Depends(require_company_id)])
...
def list_revenue(
    ...
    if company_id is not None:
        query = query.filter(TRN_Revenue.company_id == company_id)
    if case_id:
    ...
    for rev in revenues:
        case = db.query(TRN_Case).filter(TRN_Case.case_id == rev.case_id)
        if company_id is not None:
...
def get_revenue(revenue_id: int, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    rev = db.query(TRN_Revenue).filter(TRN_Revenue.revenue_id == revenue_id)
    if company_id is not None:
        rev = rev.filter(TRN_Revenue.company_id == company_id)
    rev = rev.first()
    if not rev:
...
def create_revenue(revenue_data: RevenueCreate, db: Session = Depends(get_db)):
    company_id = get_current_company_id()
    case = db.query(TRN_Case).filter(TRN_Case.case_id == revenue_data.case_id)
    if company_id is not None:
        case = case.filter(TRN_Case.company_id == company_id)
    case = case.first()
    if not case:
